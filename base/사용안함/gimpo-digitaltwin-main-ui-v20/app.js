...
const togglePanels = $('#togglePanels');
...
togglePanels.addEventListener('click', () => {
  const hidden = dashboardLayer.classList.toggle('is-hidden');
  togglePanels.textContent = hidden ? '패널 표시' : '패널 숨김';
  togglePanels.setAttribute('aria-pressed', String(!hidden));
});
